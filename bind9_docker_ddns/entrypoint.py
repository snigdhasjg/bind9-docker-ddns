...
def main():
    ...
    bind.add(DNSRecord("one", "A", "172.17.1.0", source=static_source_id))
    bind.add(DNSRecord("two", "A", "172.17.2.0", source=static_source_id))
    bind.add(DNSRecord("three", "A", "172.17.3.0", source=static_source_id))

    while True:
        LOG.info('Managed DNS entries: %s', bind.list_docker_records())
